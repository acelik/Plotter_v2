    ...
    for (int z = 0; z < denominator.size(); ++z)
    {
        ...
        for (k; k < k_max; ++k){
            ...
            for (int i = 0; i < root_files.size(); ++i){
                ...
                TF1 *f1 = new TF1("f1","pol0", 1.65, 2.45);
                f1->SetLineColor(kRed);
                effpassed-> Fit(f1, "r");

                std::string title_ = "Eff. for a simTrack to have an associated GEMRecHits in "+ numerator[k]+";"+x_axis_name_+";"+ y_axis_name_;
                
                effpassed->SetTitle(title_.c_str());

                effpassed->Draw();
                gPad-> Update();
                effpassed->GetPaintedGraph()->GetHistogram()->SetBins(1000,1.65, 2.45);
